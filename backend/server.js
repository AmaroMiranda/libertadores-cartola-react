// backend/server.js
// --- Dependências ---
require('dotenv').config(); // Carrega variáveis de ambiente do ficheiro .env para desenvolvimento local
...
app.use(cors({
  origin: [
    ...
    'https://cartola-libertadores.vercel.app/' // Adicione o seu URL da Vercel aqui
  ]
}));
...
async function fetchAndSaveScores(teamsToFetch, rounds) {
    ...
    for (const team of teamsToFetch) {
        ...
        for (const rodada of validRounds) {
            ...
            await new Promise(resolve => setTimeout(resolve, 250)); // Evita sobrecarga da API
